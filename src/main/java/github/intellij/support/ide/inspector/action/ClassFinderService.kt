...
//    }
  ...
  companion object {
    fun getInstance(project: Project): github.intellij.support.ide.inspector.action.ClassFinderService {
      return project.service()
    }

    private fun findProbableClasses(line: String): List<github.intellij.support.ide.inspector.action.ClassFinderService.ProbableClassName> {
      if (line.isBlank() || line.startsWith(github.intellij.support.ide.inspector.action.EXCEPTION_IN_THREAD) || line.startsWith(
          github.intellij.support.ide.inspector.action.CAUSED_BY
        ) || line.startsWith(github.intellij.support.ide.inspector.action.AT)) {
        return emptyList()
      }

      val result = mutableListOf<github.intellij.support.ide.inspector.action.ClassFinderService.ProbableClassName>()
      var start = -1
      var pointCount = 0
      var i = 0
      var first = true
      while (true) {
        ...
        if (start == -1 && github.intellij.support.ide.inspector.action.ClassFinderService.Companion.isJavaIdentifierStart(ch)) {
          start = i
          continue
        }
        if (start != -1 && ch == github.intellij.support.ide.inspector.action.POINT_CODE) {
          pointCount++
          continue
        }
        if (start != -1 &&
          ((line.codePointAt(i - 1) == github.intellij.support.ide.inspector.action.POINT_CODE && github.intellij.support.ide.inspector.action.ClassFinderService.Companion.isJavaIdentifierStart(
            ch
          )) ||
                  (line.codePointAt(i - 1) != github.intellij.support.ide.inspector.action.POINT_CODE && github.intellij.support.ide.inspector.action.ClassFinderService.Companion.isJavaIdentifierPart(
                    ch
                  )))) {
          val charCount = Character.charCount(ch)
          if (i + charCount >= line.length && pointCount >= 2) {
            github.intellij.support.ide.inspector.action.ClassFinderService.Companion.addProbableClass(
              line,
              start,
              line.length,
              result
        ...
        if (pointCount >= 2) {
          github.intellij.support.ide.inspector.action.ClassFinderService.Companion.addProbableClass(line, start, i, result)
        }
        pointCount = 0
        start = -1
      }
    ...
    private fun addProbableClass(line: String,
                                 startInclusive: Int,
                                 endExclusive: Int,
                                 result: MutableList<github.intellij.support.ide.inspector.action.ClassFinderService.ProbableClassName>) {
      ...
      if (github.intellij.support.ide.inspector.action.ClassFinderService.Companion.canBeShortenedFullyQualifiedClassName(
          fullClassName
        ) && github.intellij.support.ide.inspector.action.ClassFinderService.Companion.isJavaStyle(fullClassName) && !github.intellij.support.ide.inspector.action.ClassFinderService.Companion.isHardcodedNotClass(
          fullClassName
        )
      ) {
        val probableClassName = github.intellij.support.ide.inspector.action.ClassFinderService.ProbableClassName(
    ...
    private fun isHardcodedNotClass(fullClassName: String): Boolean {
      return github.intellij.support.ide.inspector.action.HARDCODED_NOT_CLASS.contains(fullClassName)
    }
