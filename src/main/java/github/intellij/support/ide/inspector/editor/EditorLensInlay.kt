package github.intellij.support.ide.inspector.editor
...
internal value class EditorLensInlay(private val inlay: Inlay<LensRenderer>) {
	...
	companion object {
		...
		fun show(editor: Editor, info: HighlightInfo): EditorLensInlay? {
			...
			val text ="a\nb\nbeansoft"
			val newLines = text.lines().map { InlineCompletionRenderTextBlock(it, LensSeverity.from(info.severity).textAttributes) }
			val newLineRender = InlineCompletionLineRenderer(editor, newLines)
//			editor.inlayModel.addBlockElement(offset, properties, newLineRender)

			val propertiesNew = InlayProperties().relatesToPrecedingText(true).disableSoftWrapping(false).priority(priority - 1)

			return editor.inlayModel.addBlockElement(offset, propertiesNew, renderer)?.let(::EditorLensInlay)
//			return null
		}
